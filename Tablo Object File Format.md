...
At the time of writing, the current format version is still `1`. DO NOT MODIFY THIS during initial development. There should be no expectation that an object file compiled with a previous build of the compiler shall be compatible with the current VM.
