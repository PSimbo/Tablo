...
mod tests {
	...
	fn tokenizes_keyword_literals() {
		...
		assert_eq!(tokens[2].kind, TokenKind::AsKeyword);
		assert_eq!(tokens[3].kind, TokenKind::AscKeyword);
		assert_eq!(tokens[4].kind, TokenKind::BoolKeyword);
		assert_eq!(tokens[5].kind, TokenKind::BreakKeyword);
		assert_eq!(tokens[6].kind, TokenKind::ByKeyword);
		assert_eq!(tokens[7].kind, TokenKind::ConstKeyword);
		assert_eq!(tokens[8].kind, TokenKind::ContinueKeyword);
		assert_eq!(tokens[9].kind, TokenKind::CountKeyword);
		assert_eq!(tokens[10].kind, TokenKind::CreateKeyword);
		assert_eq!(tokens[11].kind, TokenKind::DateKeyword);
		assert_eq!(tokens[12].kind, TokenKind::DecKeyword);
		assert_eq!(tokens[13].kind, TokenKind::DeleteKeyword);
		assert_eq!(tokens[14].kind, TokenKind::DescKeyword);
		assert_eq!(tokens[15].kind, TokenKind::ElseKeyword);
		assert_eq!(tokens[16].kind, TokenKind::FindKeyword);
		assert_eq!(tokens[17].kind, TokenKind::FirstKeyword);
		assert_eq!(tokens[18].kind, TokenKind::FnKeyword);
		assert_eq!(tokens[19].kind, TokenKind::ForKeyword);
		assert_eq!(tokens[20].kind, TokenKind::FromKeyword);
		assert_eq!(tokens[21].kind, TokenKind::GroupKeyword);
		assert_eq!(tokens[22].kind, TokenKind::IfKeyword);
		assert_eq!(tokens[23].kind, TokenKind::InKeyword);
		assert_eq!(tokens[24].kind, TokenKind::LastKeyword);
		assert_eq!(tokens[25].kind, TokenKind::LimitKeyword);
		assert_eq!(tokens[26].kind, TokenKind::MutKeyword);
		assert_eq!(tokens[27].kind, TokenKind::NewKeyword);
		assert_eq!(tokens[28].kind, TokenKind::NotKeyword);
		assert_eq!(tokens[29].kind, TokenKind::OrKeyword);
		assert_eq!(tokens[30].kind, TokenKind::OrderKeyword);
		assert_eq!(tokens[31].kind, TokenKind::PubKeyword);
		assert_eq!(tokens[32].kind, TokenKind::RecKeyword);
		assert_eq!(tokens[33].kind, TokenKind::ReturnKeyword);
		assert_eq!(tokens[34].kind, TokenKind::TextKeyword);
		assert_eq!(tokens[35].kind, TokenKind::TimeKeyword);
		assert_eq!(tokens[36].kind, TokenKind::TimestampKeyword);
		assert_eq!(tokens[37].kind, TokenKind::TimestampTzKeyword);
		assert_eq!(tokens[38].kind, TokenKind::TimeTzKeyword);
		assert_eq!(tokens[39].kind, TokenKind::TransactionKeyword);
		assert_eq!(tokens[40].kind, TokenKind::TrueKeyword);
